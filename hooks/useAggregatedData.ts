...
function buildAggregated(
  ...
  thirdParty: Partial<Record<SourceID, SourceData>>,
): Omit<AggregatedData, 'isLoading' | 'loadError'> {
  ...
  for (const sourceId of selectedSources) {
    ...
    const sourceData = thirdParty[sourceId];
    ...
    aggregatedSpellSlots.classes = {
      ...aggregatedSpellSlots.classes,
      ...sourceData.classData.SPELL_SLOTS_DATA.classes,
    };
    ...
    aggregatedAttackData.classes = {
      ...aggregatedAttackData.classes,
      ...sourceData.classData.ATTACK_DATA.classes,
    };
    aggregatedSupplementalData = {
      ...aggregatedSupplementalData,
      ...sourceData.classData.SUPPLEMENTAL_CLASS_DATA,
    };
  }
  ...
  const buildClasses = (features: ClassFeatureData[]): ClassInfo[] => {
    const resolveSaves = (className: string): SavingThrowRange[] => {
      ...
      return (aggregatedSavingThrows['Fighter'] as { saves_by_level_ranges: SavingThrowRange[] })
        .saves_by_level_ranges;
    };

    return features.map(classData => {
      const supplemental = aggregatedSupplementalData[classData.name] || {
        startingWealth: '3d6*10',
        moneyGroup: 'Martial',
      };
      ...
      const primeRequisitesEnum = classData.prime_requisites
        .map(pr => abilityStringToEnum(pr))
  ...
  return {
    CLASSES: buildClasses(allClassFeatures),
    RACES: allRaces,
    ITEMS: allItems,
...
const EMPTY: AggregatedData = {
  ...buildAggregated(new Set(['ose']), {}),
  isLoading: false,
  loadError: null,
};
...
export const useAggregatedData = (selectedSources: Set<SourceID>): AggregatedData => {
  const needsThirdParty = useMemo(
    () => [...selectedSources].some(id => id !== 'ose'),
    [selectedSources],
  );

  const [thirdParty, setThirdParty] = useState<Partial<Record<SourceID, SourceData>>>({});
  const [isLoading, setIsLoading] = useState(needsThirdParty);
  const [loadError, setLoadError] = useState<string | null>(null);

  useEffect(() => {
    let cancelled = false;
    const thirdPartyIds = [...selectedSources].filter(id => id !== 'ose');
    ...
    loadSources(thirdPartyIds)
      ...
      .catch(err => {
        if (!cancelled) {
          setLoadError(err instanceof Error ? err.message : 'Failed to load source packs');
          setIsLoading(false);
        }
      });
    return () => {
  ...
  return useMemo(() => {
    const core = buildAggregated(selectedSources, thirdParty);
    return {
      ...core,
      isLoading,
      loadError,
    };
  }, [selectedSources, thirdParty, isLoading, loadError]);
};

export { EMPTY as EMPTY_AGGREGATED };
